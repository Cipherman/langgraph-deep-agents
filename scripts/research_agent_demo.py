...
from src.graphs.react_research import agent
...
research_brief = """I want to identify and evaluate the customer persona that is surveying 化粧水 with the search keywords of 化粧水, しっとり, 乾燥肌. 
My research should focus on analyzing and comparing customer personas within Japan."""

#result = research.invoke({"researcher_messages": [HumanMessage(content=f"{research_brief}.")]})
result = agent.invoke({"messages": [{"role":"user", "content": research_brief}]})

#print(format_messages(result['researcher_messages']))
print(result)

#print("\n\n\n")
#print(format_message(result['compressed_research']))